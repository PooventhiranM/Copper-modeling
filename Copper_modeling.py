...
def streamlit_config():
    ...
    page_background_color = """<style>[data-testid="stHeader"] {background: rgba(0,0,0,0);
                                } </style>"""
...
def style_submit_button():
    st.markdown("<style>div.stButton > button:first-child {background-color: #367F89;color: white;width: 70%}</style>", 
                unsafe_allow_html=True)

# custom style for prediction result text - color and position
def style_prediction():
    st.markdown( "<style>.center-text {text-align: center;color: #20CA0C}</style>",unsafe_allow_html=True)
...
def regression():
    ...
    with st.form('Regression'):
        ...
        if button:          
            ...
            with open("C:/Users/poove/Downloads/regression_model.pkl", 'rb') as f:
                model = pickle.load(f)
...
# get input from users
    with st.form('Classification'):
        ...
        if button:         
            ...
            with open("C:/Users/poove/Downloads/classification_model.pkl", 'rb') as f:
                model = pickle.load(f)
